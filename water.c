...
Student ID: 2020110158 / 201487827
...
// NOW IT IS YOUR TURN, WRITE BELOW THE CODE FOR COMPUTING THE BILL AND UPDATING CUMULATIVE VALUES
	if (CustomerType == 1)
	{
		...
		{
		    QFreshWaterCharge = 1 + 0.35 * (QWaterConsumption - 5);
		}
		else if (QWaterConsumption > 12 && QWaterConsumption <= 25)
		{
		    QFreshWaterCharge = 3.45 + 0.50 * (QWaterConsumption - 12);
		}
		else if (QWaterConsumption > 25 && QWaterConsumption <= 40)
		{
		    QFreshWaterCharge = 9.95 + 0.75 * (QWaterConsumption - 25);
		}
		else
		{
		    QFreshWaterCharge = 21.2 + 2.50 * (QWaterConsumption - 40);
		}
	...
	sum = sum + 1;
	DomesticMoney += QAmountToPay;
	QAverageDomesticBill = DomesticMoney / sum;
